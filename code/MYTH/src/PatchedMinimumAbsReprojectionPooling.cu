...
#define PMARP_SKIP_FIRST (0)
#define MAX_CHANNELS (16)

__global__ void PatchedMinimumAbsReprojectionPooling_forward_kernel(
    float *input_bound,
    float *input_depth,
    ...
    for (int u = blockIdx.z * blockDim.z + threadIdx.z; u < W; u += blockDim.z * gridDim.z) {
        ...
        for(int n = PMARP_SKIP_FIRST; n < N; n++) {
...
extern "C" void PatchedMinimumAbsReprojectionPooling_updateOutput_gpu(
    ...
    THCudaTensor *input_mask)
{
    ...
    PatchedMinimumAbsReprojectionPooling_forward_kernel<<<grid, block, 0, stream>>>(
        input_bound_p,
        input_depth_p,
        input_features_p,
        input_mask_p,
...
__global__ void PatchedMinimumAbsReprojectionPooling_backward_kernel(
    float *input_bound,
    float *input_depth,
...
extern "C" void PatchedMinimumAbsReprojectionPooling_updateGradInput_gpu(
    THCudaTensor *input_bound,
    THCudaTensor *input_depth,
    ...
    THCudaTensor *input_mask)
{
    ...
    cudaStream_t stream = THCState_getCurrentStream(state);
    PatchedMinimumAbsReprojectionPooling_backward_kernel<<<grid, block, 0, stream>>>(
        input_bound_p,
        input_depth_p,
        dloss_input_bound_p,
        dloss_input_depth_p,
