...
__global__ void DepthReprojectionNeighbours_forward_depth_kernel(
                    float *input,
                    float *output,
                    float *cameras,
                    float *invKRs,
...
extern "C" void DepthReprojectionNeighbours_updateOutput_gpu(
    ...
    THCudaTensor *camlocs)
{
    ...
    DepthReprojectionNeighbours_forward_depth_kernel<<<grid, block, 0, stream>>>(input_depth_p, output_depth_p, cameras_p, invKRs_p, camlocs_p, batch_size, nrviews, input_height, input_width);
