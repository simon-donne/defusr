...
__global__ void DepthReprojectionCompleteBound_forward_kernel(
                    float *input,
                    float *output,
                    float *cameras,
                    float *invKRs,
...
extern "C" void DepthReprojectionCompleteBound_updateOutput_gpu(
    THCudaTensor *input,
    THCudaTensor *output,
    ...
    float dstep)
{
    ...
    DepthReprojectionCompleteBound_forward_kernel<<<grid, block, 0, stream>>>(input_p, output_p, cameras_p, invKRs_p, camlocs_p, batch_size, nrviews, input_height, input_width, dmin, dmax, dstep);
