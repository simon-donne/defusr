...
data_folder = os.path.join(base_data_folder, "dtu/")
nr_cameras = 49
nr_neighbours = 10
image_scale = 0.25
...
with open(os.path.join(output_path, "dtu_cameras.txt"), 'wt') as fh:
...
ts = []
with open(os.path.join(output_path, "dtu_images.txt"), 'wt') as fh:
...
with open(os.path.join(output_path, "dtu_points3D.txt"),'wt') as fh:
    fh.write("# nothing to see here")

# finally, create the neighbours file
with open(os.path.join(output_path, "dtu_patch-match.cfg"),'wt') as fh:
